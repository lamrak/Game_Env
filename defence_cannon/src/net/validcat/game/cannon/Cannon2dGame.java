...
public class Cannon2dGame extends GLGame {
	...
	class Camera2DScreen extends Screen {
		...
		public Camera2DScreen(IGame game) {
			...
			Assets.destroy = game.getAudio().newSound("destroy.ogg");
			Assets.gunFire = game.getAudio().newSound("gun.ogg");
		}
		...
		public void update(float deltaTime) {
			...
			if (cannon.state == Cannon.STATE_CHARGING) cannon.charging(deltaTime);
			...
			for (int i = 0; i < len; i++) {
				...
				if (cannon.state == Cannon.STATE_READY) {
